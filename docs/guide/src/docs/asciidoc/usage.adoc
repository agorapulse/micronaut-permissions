...
You can use `@ResultRequiresPermission` annotation if you want to apply the permissions check on the result of the method execution. Typical use case is the _view_ permission if fetching the object from the database is a relatively cheap operation, but you need to verify that the user have permission to view the object.

.Using `@ResultRequiresPermission`
[source,java]
----
include::{root-dir}/libs/micronaut-permissions/src/test/groovy/com/agorapulse/permissions/PostRepository.java[tags=result-requires-permission,indent=0]
----
